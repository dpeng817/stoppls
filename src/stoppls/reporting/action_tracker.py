...
class ActionTracker:
    ...
    def __init__(self, storage_path: Optional[str] = None, report_time: Optional[time] = None):
    ...
    def record_action(self, message: EmailMessage, action: RuleAction, rule_name: str):
        ...
        self.logger.debug(f"Recorded {action.type} action for message: {message.subject}")
    ...
    def generate_daily_report(self, day: Optional[date] = None, format: str = "html") -> str:
    ...
    def _generate_text_report(self, day: date, actions: List[Dict], action_counts: Dict[str, int]) -> str:
    ...
    def _generate_html_report(self, day: date, actions: List[Dict], action_counts: Dict[str, int]) -> str:
        ...
        for action_type, count in action_counts.items():
            ...
            action_type_plural = "Replies" if action_type == "reply" else f"{action_type.capitalize()}s"
    ...
    def _generate_markdown_report(self, day: date, actions: List[Dict], action_counts: Dict[str, int]) -> str:
    ...
    def send_daily_report(self, email_provider: EmailProvider, recipient_email: str, day: Optional[date] = None) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Sent daily report for {date_str} to {recipient_email}")
    ...
    def check_and_send_daily_report(self, email_provider: EmailProvider, recipient_email: str) -> bool:
        ...
        if current_time >= self.report_time and (last_report_date is None or last_report_date < today):
